...
class ParticleAnalyzerApp:
    def __init__(self, root):
        ...
        self.pan_start = None # 拖动起始点 (x, y) 像素坐标
        self.pan_ax_limits = None # 拖动开始时的轴范围
    ...
    def _init_ui(self):
        ...
        # 绑定图形交互事件
        self.canvas.mpl_connect('scroll_event', self.on_graph_scroll)       # 滚轮缩放
        self.canvas.mpl_connect('button_press_event', self.on_mouse_press)     # 点击/开始拖动
        self.canvas.mpl_connect('button_release_event', self.on_mouse_release) # 结束拖动
        self.canvas.mpl_connect('motion_notify_event', self.on_mouse_drag)     # 拖动中
    ...
    def on_graph_scroll(self, event):
        ...
        # 获取当前坐标范围 (x0, x1), (y0, y1)
        # 修正：直接获取limits端点，保留方向性，防止因排序导致Y轴翻转
        ax = self.ax_img
        x0, x1 = ax.get_xlim()
        y0, y1 = ax.get_ylim() 
    ...
    def on_mouse_press(self, event):
        ...
        if event.button == 1: # 左键：轨迹追踪选点
            self.handle_track_click(event)
        elif event.button == 3: # 右键：开始拖动视野
    ...
    def on_mouse_drag(self, event):
        ...
        if self.pan_start and event.inaxes == self.ax_img:
            ...
            # 将像素差转换为数据坐标差
            # 注意：如果拖动鼠标向右(dx>0)，视野应该向左移(xlim减小)，反之亦然
    ...
    def detect_particles(self, img_gray, ratio, min_um, max_um, threshold_val):
        ...
        if self.algo_var.get() == "Global Threshold":
            ...
            # 2. 计算合适的核大小 (Top-Hat Kernel Size)
            # 必须大于最大颗粒的直径，否则颗粒内部会被“减去”变成空心
            # 默认使用 max_um 转换为像素后放大 1.5 倍
            max_px = max_um / ratio
            ksize = int(max_px * 1.5) 
            if ksize % 2 == 0: ksize += 1 # 确保是奇数
            if ksize < 3: ksize = 3
            
            # 3. 形态学 Top-Hat 变换：原图 - 开运算图(背景)
            kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (ksize, ksize))
            tophat = cv2.morphologyEx(blurred, cv2.MORPH_TOPHAT, kernel)
            
            # 4. 对处理后的“平坦”背景图应用阈值
            # 这里的阈值滑块现在控制的是“颗粒比背景亮多少”
    ...
    def update_analysis(self, event=None, preserve_view=False):
        ...
        if preserve_view:
            # 优先使用当前图像的实时视野（因为用户可能刚刚拖动过但没来得及更新 view_limits）
        ...
        self.canvas.draw_idle() # 使用 draw_idle 优化流畅度
